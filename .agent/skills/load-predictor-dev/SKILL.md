...
- Query `NetworkStatsManager.querySummaryForDevice(ConnectivityManager.TYPE_MOBILE, subscriberId, startTime, endTime)`, passing `null` for `subscriberId` to retrieve aggregate device-wide mobile data usage. Passing an empty string `""` was found to fail silently on real hardware (matching 0 records on SM-A226B as confirmed via `TrafficStats` comparison), whereas `null` correctly matches device-wide cellular traffic across all mobile network interfaces. Do not attempt to retrieve a real `subscriberId`/IMSI — this is restricted on Android 10+ for non-carrier-privileged apps, per the dual-SIM constraint note above.
...
- **Stabilization Window & Mathematical Safety**:
  ...
  - **Minimum Meaningful Usage Threshold ($D_{used,live} \ge 10\text{ MB}$)**: To prevent incidental background traffic (sync pings, FCM heartbeats, carrier telemetry) from triggering premature confidence labels and erratic depletion projections, the engine requires a strict `AND` condition: both $T_{elapsed} \ge 1\text{ hour}$ AND $D_{used,live} \ge 10\text{ MB}$ must be satisfied before exiting `INSUFFICIENT_DATA` (Calibrating). A 10 MB threshold represents a negligible cost on prepaid allowances (~0.04% of a 24 GB pack, ~0.48% of a 2 GB pack) while granting immunity against background standby sync noise and capping early nonsensical projections.
